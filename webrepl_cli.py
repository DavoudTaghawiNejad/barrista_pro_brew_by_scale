...
SANDBOX = ""
#SANDBOX = "/tmp/webrepl/"
DEBUG = False  # Increased debug level
...
def login(ws, passwd):
    debugmsg("Starting login process")
    while True:
        c = ws.read(1, text_ok=True)
        debugmsg(f"Received login character: {c}")
        if c == b":":
            assert ws.read(1, text_ok=True) == b" "
            break
    ws.write(passwd.encode("utf-8") + b"\r")
    debugmsg("Password sent")
...
def do_repl(ws):
    ...
    class ConsolePosix:
        def __init__(self):
            ...
            self.infile = io.open(self.infd, 'rb', buffering=0)
            self.outfile = io.open(sys.stdout.fileno(), 'wb', buffering=0)
    ...
    try:
        while True:
            ...
            sel = select.select([console.infd, ws.s], [], [], 0.1)
            ...
            if c:
                if c == b"\x1d":  # ctrl-], exit
                    ...
                    debugmsg(f"Sending character: {c}")
            ...
            if ws.s in sel[0]:
                try:
                    ...
                    debugmsg(f"Error reading from WebSocket: {e}")
                    break

    except Exception as e:
        debugmsg(f"REPL error: {e}")
